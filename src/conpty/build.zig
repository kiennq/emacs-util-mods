...
pub fn build(b: *std.Build) void {
    ...
    if (target.result.os.tag != .windows) {
        std.debug.panic("conpty is Windows-only", .{});
    }
    ...
    const check = b.step("check", "Check that the conpty module compiles");
    check.dependOn(&conpty_check_obj.step);

    const conpty_test_mod = b.createModule(.{
        .root_source_file = b.path("conpty.zig"),
        .target = target,
        .optimize = optimize,
        .link_libc = true,
    });
    addModuleIncludes(conpty_test_mod, emacs_include);
...
fn addModuleIncludes(mod: *std.Build.Module, emacs_include: std.Build.LazyPath) void {
